...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit attaop123/reddit:latest "
                       sh "docker push attaop123/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image attaop123/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.133.155.252 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.145.112.52:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#joe1-devsecops-cicd-alerts', //update and provide your channel name
